...
We will be implementing all our functions in `linearalgebra.cpp` and `linearalgebra.h`. remember you should put all declarations in the `.h` and all the implementations in the `.cpp` file.
...
# Finally
As mentioned before, keep all your implementations in `linearalgebra.cpp` and `linearalgebra.h`. do not alter other files at all. In case you want to test your code you may only use the `debug` section of the `main.cpp` which will be explained in the TA class.
