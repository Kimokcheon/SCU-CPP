...
Mocha::Mocha(const Mocha& mo)
{
	this->name = mo.name;
	for(const auto& i : mo.ingredients)
	{
	...
	for(const auto& i : mo.side_items)
	{
		Ingredient* temp = nullptr;
...
{
	side_items.clear();
	std::cout << "Mocha destructor called" << std::endl;
}
void Mocha::operator=(const Mocha& mo)
{
	if(this == &mo) return;
